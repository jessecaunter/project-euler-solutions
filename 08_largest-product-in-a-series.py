# The four adjacent digits in the 1000-digit number that have the greatest product are 9 x 9 x 8 x 9 = 5832

input_sequence = 

# Find the thirteen adjacent digits in the 1000-digit number that have the greatest product.
num_adj_digits = 13

# What is the value of this product?

# --------------------------------------------------------------------------------------------------

str_sequence = str(input_sequence)

iterator = 0
current_combo = []
for i in range(num_adj_digits):
    current_combo.append(str_sequence[i])
    iterator += 1

# Important to initialise this with a copy
# Otherwise it won't hold it's value when current_combo changes
greatest_combo = current_combo.copy()

def next_combo(i):
    current_combo.append(str_sequence[i])
    current_combo.pop(0)

def string_to_int(str_list):
    int_list = []
    for i in range(len(str_list)):
        int_list.append(int(str_list[i]))
    return int_list

def calc_product(str_list):
    int_list = string_to_int(str_list)
    product = 1
    for i in range(len(int_list)):
        product *= int_list[i]
    return product

current_product = calc_product(current_combo)
greatest_product = calc_product(greatest_combo)

def is_current_product_greater(current_product, greatest_product):
    if current_product > greatest_product:
        return True
    else:
        return False

for i in range(iterator, len(str_sequence)):
    next_combo(i)
    current_product = calc_product(current_combo)
    if is_current_product_greater(current_product, greatest_product):
        greatest_product = current_product
        greatest_combo = current_combo.copy()

print(f'Combination of {num_adj_digits} adjacent digits which make the greatest product:\n {greatest_combo}')
print(f'PRODUCT: {greatest_product}')